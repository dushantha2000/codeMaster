...
<head>
    ...
    <title>Snippet Pro | Create</title>
...
<body class="text-gray-100" x-data="snippetEditor()" x-cloak>
    ...
    <form action="{{ url('snippet-store') }}" method="POST" class="h-full flex flex-col">
        ...
        <header class="h-20 flex items-center justify-between px-4 md:px-8 shrink-0 glass-card border-b border-white/5">
            ...
            <div class="flex items-center gap-2 md:gap-4">
                <select name="language" 
                    ...
                    <option value="js">⚛️ React</option>
        ...
        <div class="flex-1 flex overflow-hidden p-2 md:p-4 gap-2 md:gap-4">
            ...
            <!-- Sidebar - Files List -->
            <aside class="sidebar-container w-80 glass-card rounded-xl md:rounded-2xl flex flex-col overflow-hidden shadow-2xl"
                   :class="{ 'mobile-open': mobileMenuOpen }">
                <!-- Sidebar Header -->
                <div class="p-4 md:p-5 border-b border-white/5 bg-white/5">
                    <div class="flex justify-between items-center mb-3 md:mb-4">
                        <h2 class="text-xs md:text-sm font-bold uppercase tracking-wider text-gray-400 flex items-center gap-2">
                        ...
                        <div class="flex items-center gap-2">
                            <span class="text-xs px-2 py-1 rounded-full bg-white/10 text-gray-300 font-semibold" x-text="files.length"></span>
                            <button type="button" 
                                    @click="toggleMobileSidebar"
                    ...
                    <button type="button" 
                            @click="showAddFileModal = true; mobileMenuOpen = false" 
                ...
                <div class="flex-1 overflow-y-auto p-2 md:p-3 space-y-2">
                    <template x-for="(file, index) in files" :key="index">
                        <div @click="activeTab = index; mobileMenuOpen = false" 
                             :class="activeTab === index ? 'tab-active text-white scale-105' : 'bg-white/5 text-gray-400 hover:bg-white/10'"
                            ...
                            <div class="flex items-center gap-2 md:gap-3 flex-1 min-w-0">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div x-text="file.name || 'untitled.php'" class="text-xs md:text-sm font-semibold truncate"></div>
                                    <div x-text="file.path || 'root'" class="text-[10px] md:text-xs opacity-60 truncate"></div>
                                </div>
                            </div>
                            <button type="button" 
                    ...
                    <div x-show="activeTab === index" class="h-full flex flex-col">
                        ...
                        <div class="p-4 md:p-6 border-b border-white/5 bg-white/5">
                            <div class="file-meta-mobile grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                                <div>
                                    <label class="text-[10px] md:text-xs font-bold text-gray-400 block mb-2 flex items-center gap-2">
                        ...
                        <div class="flex-1 relative code-editor">
                            ...
                            <!-- Line Numbers -->
                            <div x-show="showLineNumbers" class="absolute left-0 top-0 bottom-0 w-8 md:w-12 bg-black/30 border-r border-white/5 text-gray-600 text-[10px] md:text-xs code-font pt-4 md:pt-8 select-none pointer-events-none">
    ...
    <!-- Add File Modal -->
    <div x-show="showAddFileModal" 
         x-cloak
         @click.self="showAddFileModal = false"
         class="fixed inset-0 z-50 flex items-center justify-center modal-backdrop p-4">
        <div @click.away="showAddFileModal = false"
            ...
            <div class="space-y-4 md:space-y-5">
                ...
                <div>
                    <label class="text-xs md:text-sm font-semibold text-gray-300 block mb-2">File Path (Optional)</label>
                    <input type="text" 
                ...
                <!-- Quick Templates -->
                <div>
                    <label class="text-xs md:text-sm font-semibold text-gray-300 block mb-2 md:mb-3">Quick Templates</label>
                    <div class="grid grid-cols-3 gap-2">
                        <button type="button" 
                                @click="newFile.name = 'index.php'" 
                        ...
                        <button type="button" 
                                @click="newFile.name = 'script.js'" 
                                class="px-2 md:px-3 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-[10px] md:text-xs font-semibold transition-all border border-white/10 hover:border-white/20">
                            JS
                        </button>
                        <button type="button" 
                                @click="newFile.name = 'style.css'" 
                                class="px-2 md:px-3 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-[10px] md:text-xs font-semibold transition-all border border-white/10 hover:border-white/20">
                            CSS
                        </button>
                    </div>
            ...
            <div class="space-y-3 md:space-y-4">
                <div class="flex items-center justify-between p-3 md:p-4 bg-white/5 rounded-lg border border-white/5">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <div class="w-11 h-6 bg-white/10 peer-focus:ring-2 peer-focus:ring-white/20 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-white/30"></div>
                    </label>
                </div>
    ...
    <div x-show="deleteConfirm !== null" 
        ...
        <div x-transition
            ...
            <div class="text-center">
                <div class="mx-auto w-12 h-12 md:w-16 md:h-16 bg-white/10 rounded-full flex items-center justify-center mb-3 md:mb-4 border border-white/10">
            ...
            return {
                ...
                files: [
                    { 
                        name: 'web.php', 
                        path: 'routes/', 
                        content: '<?php\n\nRoute::get("/", function() {\n    return "Welcome to Laravel iew("welcome");\n});' 
                    }
                ],
