...
@section('content')
    <div class="w-full max-w-6xl mx-auto px-4">
        ...
        <nav class="flex mb-6" aria-label="Breadcrumb">
            <ol class="flex items-center space-x-2">
                ...
                <li><a href="{{ url('/settings') }}"
                        class="text-gray-500 hover:text-gray-700 transition font-medium">Profile</a></li>
        ...
        <div class="mb-12">
            ...
            <p class="text-gray-400 text-sm font-medium">Manage cross-vault access and collaborate with verified security
                partners.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 pb-12">
            ...
            <div class="lg:col-span-4 space-y-6">
                <div class="sticky top-20 space-y-6">
                    <div
                        class="glass-card rounded-3xl p-8 border border-white/10 shadow-2xl relative overflow-hidden text-center group">

                        {{-- Profile Image Section --}}
                        <div class="relative inline-block mb-6">
                            <div
                                class="w-32 h-32 rounded-3xl bg-blue-500/5 border border-blue-500/20 flex items-center justify-center mx-auto shadow-lg overflow-hidden transition-transform duration-500 group-hover:scale-[1.05]">
                                @if (Auth::user()->profile_image)
                                    <img src="{{ asset('profileImages/' . Auth::user()->profile_image) }}"
                                        alt="{{ Auth::user()->name }}" class="w-full h-full object-cover">
                                @else
                                    <span
                                        class="text-5xl font-black text-blue-500 opacity-40">{{ substr(Auth::user()->name, 0, 1) }}</span>
                                @endif
                            </div>
                            <div
                                class="absolute -bottom-2 -right-2 bg-blue-600 text-white text-[10px] font-bold px-3 py-1.5 rounded-xl border-4 border-[#09090b] shadow-2xl tracking-widest uppercase">
                                Vault 1
                            </div>
                        </div>
                        ...
                        <div class="mt-6 pt-6 border-t border-white/5">
                            <a href="{{ url('/settings') }}"
                                class="bg-blue-500/10 hover:bg-blue-500/20 w-full text-sm font-bold py-2 rounded-xl block transition">
                                Vault Settings
                            </a>
                        </div>
                    </div>
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class=" rounded-3xl p-8 border border-white/10 relative z-20">
                    ...
                    <form action="{{ url('/user/partnerships') }}" method="POST" class="space-y-6">
                        ...
                        <div class="relative">
                            <label class="block text-xs font-bold text-gray-400 tracking-widest  mb-3 ml-1">Search Verified
                                Partners</label>
                            ...
                            <div class="flex flex-col md:flex-row items-center gap-4">
                                <div class="relative group w-full">
                                    ...
                                    {{-- Suggestions --}}
                                    <div id="suggestions"
                                        class="absolute left-0 right-0 z-[1000] mt-2 bg-[#0A0A0A] border border-white/10 rounded-xl shadow-2xl hidden max-h-64 overflow-y-auto divide-y divide-white/5 backdrop-blur-xl">
                                    </div>
                                </div>

                                <button type="submit"
                                    class="btn-primary inline-flex items-center justify-center gap-2 text-sm py-2 px-8 rounded-xl font-bold whitespace-nowrap shadow-lg">
                                    Execute Access
                                </button>
                            </div>
                    ...
                    <div class="mt-10 pt-8 border-t border-white/5">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <span
                                class="px-2.5 py-1 rounded-lg text-[10px] font-bold tracking-widest  text-blue-400 ">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @forelse($partners as $partner)
                                <div
                                    class="group relative flex items-center justify-between p-4 rounded-xl glass-card border-white/5 hover:border-blue-500/30 transition-all duration-300 shadow-sm">
                                    <div class="flex items-center min-w-0">
                                        <div class="relative flex-shrink-0">
                                            <div
                                                class="w-12 h-12 rounded-lg overflow-hidden bg-blue-500/10 flex items-center justify-center text-blue-400 font-black text-xl border border-blue-500/20">
                                        ...
                                        <div class="ml-4 overflow-hidden">
                                            ...
                                            <span
                                                class="text-[10px] font-bold uppercase tracking-widest text-gray-500 mt-0.5 block">
                                    ...
                                    <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button type="button"
                                            ...
                                            data-profile-image="{{ $partner->profile_image}}">
                            ...
                            @empty
                                <div class="col-span-full flex flex-col items-center justify-center py-10 text-center">
                                    ...
                                    <p class="text-[10px] font-bold  tracking-widest text-gray-500">No Active
                                        Collaborators</p>
            ...
            <div onclick="event.stopPropagation()"
                ...
                <div class="p-8">
                    ...
                    <div class="flex items-center gap-4 mb-8">
                        <div id="modalPartnerImageContainer" class="w-16 h-16 rounded-xl overflow-hidden bg-blue-500/10 flex items-center justify-center border border-blue-500/20 shadow-sm transition-all duration-300">
                            <img id="modalPartnerImage" src="" alt="" class="w-full h-full object-cover hidden">
                            <span id="modalPartnerInitials" class="text-blue-400 font-black text-2xl"></span>
                        </div>
                        <div>
                            <h4 id="modalPartnerNameDisplay" class="text-white font-black text-lg tracking-tight">Partner
                                Entity</h4>
                    ...
                    <form action="{{ url('/partners/update') }}" method="POST" class="space-y-6">
                        ...
                        <div class="space-y-3">
                            <label
                                class="flex items-center justify-between cursor-pointer group p-4 rounded-xl bg-[#050505] border border-white/5 hover:border-blue-500/30 transition-all">
                                <div>
                                    <span class="block text-sm font-bold text-white tracking-tight">Read Only Access</span>
                                    <span class="block text-xs text-gray-500 font-medium mt-1">Can view and retrieve vault
                                        snippets.</span>
                                </div>
                                <div class="relative">
                            ...
                            <label
                                ...
                                <div>
                                    ...
                                    <span class="block text-xs text-gray-500 font-medium mt-1">Can modify, create, and
                                        commit logic changes.</span>
                                </div>
                                <div class="relative">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Close modal with explicit button listener to bypass propagation stop
        ...
        document.addEventListener('click', function (e) {
            ...
            if (editBtn) {
                ...
                const profileImage = editBtn.getAttribute('data-profile-image');
                ...
                const modalImg = document.getElementById('modalPartnerImage');
                const modalInitials = document.getElementById('modalPartnerInitials');

                if (profileImage && profileImage.trim() !== '') {
                    modalImg.src = "{{ asset('profileImages') }}/" + profileImage;
                    modalImg.classList.remove('hidden');
                    modalInitials.classList.add('hidden');
                } else {
                    modalImg.classList.add('hidden');
                    modalInitials.textContent = name.charAt(0).toUpperCase();
