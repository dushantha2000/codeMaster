...
@section('title', 'Create Logic Node')

@section('content')

    <div class="h-full flex flex-col bg-[#030303]">
        <form action="{{ url('/snippet-store') }}" method="POST" class="h-screen flex flex-col overflow-hidden">
            {{ csrf_field() }}
            ...
            <!-- Default UI Header -->
            <header class="h-20 flex items-center justify-between px-4 md:px-8 shrink-0 glass-card border-b border-white/5 sticky top-0 z-30">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="w-8 h-8 bg-black border border-white/10 rounded-lg flex items-center justify-center shadow-lg shadow-blue-600/20">
                ...
                <div class="flex-1 flex items-center justify-center gap-4 max-w-xl px-4 min-w-0 group cursor-pointer" @click="showProjectModal = true">
            ...
            <div class="flex-1 flex overflow-hidden p-2 md:p-6 gap-2 md:gap-4">
                ...
                <!-- Editor Workspace -->
                <main class="flex-1 glass-card rounded-xl flex flex-col overflow-hidden shadow-2xl border border-white/5 relative z-10">
                    <template x-for="(file, index) in files" :key="index">
        ...
        <template x-if="true">
            <div>
                ...
                <div x-show="showInitialModal" x-cloak
                    ...
                    <div class="glass-card rounded-2xl p-8 md:p-10 w-full max-w-xl shadow-2xl border border-white/10 animate-in zoom-in-95 duration-300">
                        <div class="mb-8">
                            ...
                            <p class="text-sm text-gray-500">Initialize your snippet with primary metadata.</p>
                        </div>
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <p x-show="projectValidation.title" class="text-xs text-red-500 mt-1 font-medium" x-text="projectValidation.title"></p>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea x-model="projectInfo.description" rows="3"
                                    placeholder="Briefly describe the logic"
                                    class="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-all resize-none shadow-inner"
                                    :class="{ 'border-red-500/30': projectValidation.description }"></textarea>
                                <p x-show="projectValidation.description" class="text-xs text-red-500 mt-1 font-medium" x-text="projectValidation.description"></p>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select x-model="projectInfo.language"
                                        class="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-all cursor-pointer shadow-inner">
                                ...
                                <div>
                                    ...
                                    @if ($categories->isEmpty())
                                        <div class="h-[46px] flex items-center px-4 bg-black/20 rounded-lg border border-dashed border-white/10">
                                            <a href="{{ url('/categories.index') }}" class="text-xs text-blue-500 hover:text-blue-400 font-bold transition-colors">Setup Category</a>
                                        </div>
                                    @else
                                    ...
                                    @endif
                                    <p x-show="projectValidation.category" class="text-xs text-red-500 mt-1 font-medium" x-text="projectValidation.category"></p>
                                </div>
                            </div>
                        </div>

                        <div class="flex gap-4 mt-10">
                            <a href="{{ url('/') }}" class="flex-1 bg-white/5 hover:bg-white/10 text-gray-500 hover:text-white px-6 py-3 rounded-lg text-sm font-bold transition-all border border-white/5 text-center">Cancel</a>
                            <button type="button" @click="submitProjectInfo" class="flex-1 btn-primary px-6 py-3 rounded-lg text-sm font-bold h-[48px] shadow-lg shadow-blue-900/10">Start Entry</button>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="showProjectModal" x-cloak @click.self="showProjectModal = false"
                    ...
                    <div class="glass-card rounded-2xl p-8 w-full max-w-xl shadow-2xl border border-white/10 animate-in slide-in-from-bottom-4 duration-300">
                        <div class="flex items-center justify-between mb-8">
                            <h3 class="text-xl font-bold text-white">Project Specs</h3>
                            <button @click="showProjectModal = false" class="text-gray-500 hover:text-white transition-colors text-2xl">&times;</button>
                        </div>

                        <div class="space-y-6">
                            <div>
                                <label class="text-xs font-bold text-gray-500 mb-2 block ml-1">Title</label>
                                <input type="text" x-model="projectInfo.title" required class="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-all">
                            </div>
                            <div>
                                <label class="text-xs font-bold text-gray-500 mb-2 block ml-1">Description</label>
                                <textarea x-model="projectInfo.description" required rows="3" class="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-all resize-none shadow-inner"></textarea>
                            </div>
                            <div class="grid grid-cols-2 gap-6">
                                <div>
                                    <label class="text-xs font-bold text-gray-500 mb-2 block ml-1">Language</label>
                                    <select x-model="projectInfo.language" class="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none appearance-none">
                                        <option value="laravel">Laravel</option>
                                        <option value="tailwind">Tailwind CSS</option>
                                        <option value="react">React</option>
                                        <option value="vue">Vue.js</option>
                                ...
                                <div>
                                    <label class="text-xs font-bold text-gray-500 mb-2 block ml-1">Category</label>
                                    <select x-model="projectInfo.category" class="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none appearance-none">
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->category_id }}">{{ $category->category_name }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="flex gap-4 mt-10">
                            <button type="button" @click="showProjectModal = false" class="flex-1 bg-white/5 hover:bg-white/10 text-gray-500 hover:text-white px-6 py-3 rounded-lg text-sm font-bold transition-all border border-white/5">Close</button>
                            <button type="button" @click="showProjectModal = false" class="flex-1 btn-primary px-6 py-3 rounded-lg text-sm font-bold h-[48px]">Update Meta</button>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="showAddFileModal" x-cloak @click.self="showAddFileModal = false"
                    class="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
                    <div class="glass-card rounded-2xl p-8 w-full max-w-md shadow-2xl border border-white/10 animate-in zoom-in-95 duration-300">
                        <div class="flex items-center justify-between mb-8">
                            <h3 class="text-xl font-bold text-white">New Logic Node</h3>
                            <button @click="showAddFileModal = false" class="text-gray-500 hover:text-white transition-colors text-2xl">&times;</button>
                        </div>

                        <div class="space-y-6">
                            <div>
                                <label class="text-xs font-bold text-gray-500 mb-2 block ml-1">Node Name</label>
                                <input type="text" x-model="newFile.name" @keyup.enter="addFile" required placeholder="e.g. LogicController.php"
                                    class="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-all">
                            </div>
                            <div>
                                <label class="text-xs font-bold text-gray-500 mb-2 block ml-1">Logic Path</label>
                                <input type="text" x-model="newFile.path" @keyup.enter="addFile" placeholder="e.g. app/Http/Logic" required
                                    class="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-all">
                            </div>
                        </div>

                        <div class="flex gap-4 mt-10">
                            <button type="button" @click="showAddFileModal = false" class="flex-1 bg-white/5 hover:bg-white/10 text-gray-500 hover:text-white px-6 py-3 rounded-lg text-sm font-bold transition-all border border-white/5">Cancel</button>
                            <button type="button" @click="addFile" class="flex-1 btn-primary px-6 py-3 rounded-lg text-sm font-bold h-[48px]">Register Node</button>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="deleteConfirm !== null" x-cloak @click.self="deleteConfirm = null"
                    ...
                    <div class="glass-card rounded-2xl p-8 w-full max-w-sm shadow-2xl border border-white/10 text-center animate-in zoom-in-95 duration-300">
                        ...
                        <div class="flex flex-col gap-2">
                            <button @click="removeFile(deleteConfirm)" class="w-full bg-red-600 hover:bg-red-500 text-white py-3 rounded-lg font-bold text-sm transition-all shadow-lg">Confirm Delete</button>
                            <button @click="deleteConfirm = null" class="w-full py-3 bg-white/5 hover:bg-white/10 text-gray-500 hover:text-white rounded-lg font-bold text-sm transition-all">Cancel</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        function snippetEditor() {
            return {
                ...
                submitProjectInfo() {
                    this.projectValidation = { title: '', description: '', category: '' };
                    let isValid = true;

                    if (!this.projectInfo.title.trim()) {
                        this.projectValidation.title = 'Title Required';
                        isValid = false;
                    }
                    if (!this.projectInfo.description.trim()) {
                        this.projectValidation.description = 'Description Required';
                        isValid = false;
                    }
                    if (!this.projectInfo.category) {
                        this.projectValidation.category = 'Category Required';
                        isValid = false;
                    }
